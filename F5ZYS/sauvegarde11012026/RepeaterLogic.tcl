...
#
#Message d'accueil selon l'heure Bonjour ou Bonsoir
#

set heure [clock format [clock seconds] -format "%H%M"]
if {  ($heure >= 1800) || ($heure <= 0300) } {
    playMsg $mycall "bonsoir";
   # spellWord $mycall;
    puts "Ouverture bonsoir";
...
#set fi [open "/mnt/ram/smeter.tcl" w]
#puts $fi "set signal -1;"
#close $fi
...
if { [file exists /etc/svxlink/network]  } {
  ...
  if  [string match "*tec*" $salon] then {
                ...
                playMsg "Spotnik" "Stec";
  }
  if  [string match "*loc*" $salon] then {
                puts "Salon Local";
                playMsg "Spotnik" "Sloc";
        }
  ...
  if  [string match "*bav*" $salon] then {
                ...
                playMsg "Spotnik" "Sbav";
  }
  if  [string match "*fon*" $salon] then {
                puts "Reseau FON";
                playMsg "Spotnik" "Sfon";
  }
...
if { [file exists /sys/class/gpio/gpio24/value]  } {
        ...
        if  {$absence_secteur == "1"} {
          puts "230V : NO"
          playMsg "SVXCard/Mesurement" "PowerSupplyNO";
...
#puts "Lecture de tension dans 10s...";
#set runcmd [exec python3 /home/mesureV/mesureVtempo.py &];
...
set heure [clock format [clock seconds] -format "%H%M"]
if {  ($heure >= 2200) || ($heure <= 0100) } {
    playMsg $mycall "bonnenuit";
    puts "Fermeture bonne nuit";
} else {
  playMsg $mycall "aurevoir";
    puts "Fermeture aurevoir";
}
CW::play "VA";
playSilence 500;
CW::play "K";
